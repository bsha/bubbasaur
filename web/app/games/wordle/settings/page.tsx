      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-6">
        <header className="space-y-2">
          ...
          <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">
            Customize your next game
          </h1>
          <p className="max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
            Choose a word length, enter a custom answer, and select how many
              ...
              <div className="rounded-xl border border-border/70 bg-background/70 p-3 transition-colors sm:p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex min-w-0 flex-1 items-start gap-3">
                    <RadioGroupItem
                      value="letterCount"
                      id="letterCountOption"
                      className="mt-0.5"
                    />
                    <div className="min-w-0 flex-1">
                      <label
                        htmlFor="letterCountOption"
                        className="text-sm font-medium leading-5 text-foreground"
                      >
                        Set number of letters
                      </label>
                      <p className="mt-1 text-sm leading-5 text-muted-foreground">
                        Pick a standard word length for the board.
                      </p>
                      {mode === "letterCount" && (
              ...
              <Field className="flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Input
                    ...
                    value={guessCount}
                    onChange={(event) =>
                      handleNumberInput(event.target.value, setGuessCount)
                    }
                    className="h-10 w-24"
                  />
